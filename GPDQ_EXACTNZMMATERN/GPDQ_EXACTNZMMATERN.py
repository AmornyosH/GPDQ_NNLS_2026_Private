...
# ================================== Others ==================================
from GPDQ_EXACTNZMMATERN import my_exact_gp
    ...
    def getAlteredObservation_old(self, states):
        ...
        with torch.no_grad():
            ...
            for m in range(len(states)):
                ...
                # Get norm 
                _norm_a = self.TEMPERATURE_COEFF * torch.sum(torch.square(_beh_a_n_tensor - a_i_m_1), dim=1).reshape(_sampling_size, 1)
        ...
        while self.training_record < total_epoch:
            ...
            for g in range(_num_gradient_step):
                ...
                if _actor_update_flag:
                    ...
                    # # 2
                    # _a_beh = self.predict(state=batch_state_tensor, size=_batch_size, guide=False, target=False).view(-1, self.ACTION_DIM)
                    # _a_pred = self.gp_model.mean(batch_state_tensor)
                    # _q_pred = self.q_1(torch.concat([batch_state_tensor, _a_pred], dim=1))
                    # _q_norm = _q_pred
                    # _gp_mean_loss = -_q_norm.mean() + torch.sum(3 * (torch.square(_a_beh - _a_pred)), dim=1).mean()

                    _gp_mean_loss.backward()
            ...
            if self.training_record % 5 == 0:
                self.gp_model.y_train = self.getAlteredObservation(self.gp_model.x_train, self.gp_model.y_train_org)

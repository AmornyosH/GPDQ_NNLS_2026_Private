...
# ================================== Others ==================================
from GPDQ_EXACTNZMRBF2 import my_exact_gp
        ...
        while self.training_record < total_epoch:
            ...
            for g in range(_num_gradient_step):
                ...
                if _actor_update_flag:
                    ...
                    _q_beh = self.q_1(torch.concat([batch_state_tensor, _a_beh], dim=1))
                    _q_norm = _q_pred / _q_beh
                    _gp_mean_loss = -_q_norm.mean() + (1 * (torch.square(_a_beh - _a_pred)).mean())

...
# ================================== Others ==================================
from GPDQ_EXACTNZMRBF3 import my_exact_gp
...
class GaussianProcessDiffusionQlearning:
    def __init__(self, params_dict:dict, dataset:dict, ft:bool=False):
        ...
        if _ans_1 == 'n' or _ans_1=='N' or _ans_1=='No' or _ans_1=='NO':
            ...
            # # Load pretrained behaviour model to gp_mean
            # self.gp_model.mean.load_state_dict(_load_beh['epsilon_beh'])
            # self.gp_model.mean_optimizer.load_state_dict(_load_beh['epsilon_beh_optimizer'])
            # self.gp_model.mean_train = self.reverseProcess(inputs=self.gp_model.x_train, 
            #                                                size=self.gp_model.gp_training_size, 
            #                                                guide=False, mean_fn=True)
    ...
    def reverseProcess(self, inputs:list, size:int, guide:bool=False, target:bool=False, mean_fn:bool=False):
        ...
        for i in range(_diffu_steps): 
            ...
            if not mean_fn:
                ...
                epsilon_theta_t = self.gp_model.mean(torch.concat((inputs, x_T, rev_pos_emb), dim=1))
    ...
    def getAlteredObservation_old(self, states):
    ...
    def getAlteredObservation(self, states, actions):
        with torch.no_grad():
            _sampling_size = self.num_action_candidates
            ...
            # Norm a
            _norm_a = self.TEMPERATURE_COEFF * torch.sum(torch.square(actions_rep - a_all), dim=2).view(batch_size, _sampling_size)
    ...
    def training(self, total_epoch:int, eval:bool=False, ft:bool=False):
        ...
        while self.training_record < total_epoch:
            ...
            _actor_update_flag = True if (self.training_record % 2) == 0 else False
            ...
            for g in range(_num_gradient_step):
                ...
                if _actor_update_flag:
                    ...
                    # 1
                    _a_true = self.getAlteredObservation(states=batch_state_tensor, actions=batch_action_tensor)  # Must be new function from Claude.
                    # _a_pred = self.reverseProcess(inputs=batch_state_tensor, size=_batch_size, guide=False, mean_fn=True)
                    _a_pred = self.gp_model.mean(batch_state_tensor)
                    _gp_mean_loss = torch.mean(torch.square(_a_true - _a_pred), dim=1, keepdim=True)
                    _gp_mean_loss = _gp_mean_loss.mean()

                    # # 2
                    # _gp_mean_residual_noise = self.gp_model.mean(torch.concat([batch_state_tensor, forward_action_tensor, encode_t_tensor], dim=1))
                    # _gp_mean_diffu_loss = torch.mean(torch.square(epsilon_tensor - _gp_mean_residual_noise), dim=1, keepdim=True)
                    # _a_pred = self.reverseProcess(inputs=batch_state_tensor, size=_batch_size, guide=False, mean_fn=True)
                    # _q_pred = self.q_1(torch.concat([batch_state_tensor, _a_pred], dim=1))
                    # _q_norm = _q_pred / torch.abs(self.q_1(torch.concat([batch_state_tensor, batch_action_tensor], dim=1)))
                    # # _q_norm = _q_pred - (self.TEMPERATURE_COEFF * torch.sum(torch.square(_a_pred - batch_action_tensor), dim=1))
                    # _gp_mean_loss = -_q_norm.mean() + _gp_mean_diffu_loss.mean()
            ...
            # _gp_loss = self.myGPTraining(total_epoch=10 if self.gp_model_type == 'sparse' else 10, ft=False)

            # Update Altered observation for every ... epoch.
            if self.training_record % 5 == 0:
                self.gp_model.y_train = self.getAlteredObservation(self.gp_model.x_train, actions=self.gp_model.y_train_org)
    ...
    # def predictGP(self, x_test, grad=False):
    #     x_test = x_test[:, 0:self.gp_model.x_dim]
    #     x_test = x_test.view(-1, self.gp_model.x_dim)

    #     # _mean_x_train = self.reverseProcess(inputs=self.gp_model.x_train, size=self.gp_model.gp_training_size, guide=False, mean_fn=True)
    #     _mean_x_train = self.gp_model.mean_train
    #     _mean_x_test = self.reverseProcess(inputs=x_test, size=len(x_test), guide=False, mean_fn=True)

    #     if not grad:
    #         with torch.no_grad():
    #             _k_s = self.gp_model.rbfKernel(X_1=self.gp_model.x_train, X_2=x_test, noise=False)
    #             _k_ss = self.gp_model.rbfKernel(X_1=x_test, X_2=x_test, noise=False)
    #             # Cholesky decomposition
    #             _L = torch.linalg.cholesky(self.gp_model.K, upper=False)
    #             _alpha = torch.linalg.solve_triangular(_L.T, torch.linalg.solve_triangular(_L, (self.gp_model.y_train - _mean_x_train), upper=False), upper=True)
    ...
    # def myGPTraining(self, total_epoch:int, ft:bool=False):
    #     _batch_size = self.gp_model.gp_training_size # H
    #     _gradient_step = self.gp_model.gp_training_size // _batch_size

    #     _training_record = 0
    #     while _training_record < total_epoch:
    #         # _start_time = time()
    #         for g in range(_gradient_step):
    #             _batch_x = self.gp_model.x_train[g*_batch_size:_batch_size+(g*_batch_size)]
    #             _batch_y = self.gp_model.y_train[g*_batch_size:_batch_size+(g*_batch_size)]

    #             self.gp_model.optimizer.zero_grad()
    #             _k = self.gp_model.rbfKernel(X_1=_batch_x, X_2=_batch_x, noise=True)

    #             _mean = self.reverseProcess(inputs=self.gp_model.x_train, size=_batch_size, guide=False)
    ...
    #             self.gp_model.optimizer.step()

    #             self.gp_model.mll_append.append(_mll.tolist())

    #         _training_record += 1
            
    #     self.gp_model.K = self.gp_model.rbfKernel(X_1=self.gp_model.x_train, X_2=self.gp_model.x_train, noise=True)
    #     # self.mean_train = self.reverseProcess(inputs=self.gp_model.x_train, size=self.gp_model.gp_training_size, guide=False, mean_fn=True)

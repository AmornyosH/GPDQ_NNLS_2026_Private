...
def getParamsDict(env):
    return {'simple_sine' : {'environment': 'simple_sine', 'horizon': (4*np.pi//0.05)//2, 'gp_num_sample': 125, 'gp_num_inducing': 25, 'gp_batch_size': 25, 'state_dim': 1, 'action_dim': 1, 'normalise_reward': False,'diffusion_step': 25, 'task': TASK}, 
            ...
            'walker2d-medium-replay-v2' :  {'environment': 'walker2d-v2-medium-expert', 'horizon': 1000, 'gp_num_sample': 1000, 'gp_num_inducing': 100, 'gp_batch_size': 256, 'state_dim': 17, 'action_dim': 6, 'normalise_reward': True, 'diffusion_step': 5, 'task': TASK},
            'walker2d-medium-v2' :  {'environment': 'walker2d-medium-v2', 'horizon': 1000, 'gp_num_sample': 1500, 'gp_num_inducing': 1000, 'gp_batch_size': 256, 'state_dim': 17, 'action_dim': 6, 'normalise_reward': True, 'diffusion_step': 5, 'task': TASK},
            ...
            'hopper-medium-expert-v2' :  {'environment': 'hopper-medium-expert-v2', 'horizon': 1000, 'gp_num_sample': 1000, 'gp_num_inducing': 1000, 'gp_batch_size': 256, 'state_dim': 11, 'action_dim': 3, 'normalise_reward': True, 'diffusion_step': 5, 'task': TASK},
            ...
            'hopper-medium-v2' :  {'environment': 'hopper-medium-v2', 'horizon': 1000, 'gp_num_sample': 1000, 'gp_num_inducing': 1000, 'gp_batch_size': 256, 'state_dim': 11, 'action_dim': 3, 'normalise_reward': True, 'diffusion_step': 5, 'task': TASK},
            ...
            'halfcheetah-medium-v2' :  {'environment': 'halfcheetah-medium-v2', 'horizon': 1000, 'gp_num_sample': 2000, 'gp_num_inducing': 1000, 'gp_batch_size': 256, 'state_dim': 17, 'action_dim': 6, 'normalise_reward': True, 'diffusion_step': 5, 'task': TASK},
...
def interactionMuJoCo(agent, seed, random:bool=False) -> float:
    ...
    while not terminated and not truncated if not UNCERTAINTY else not truncated:
        if not random:
            ...
            # action = agent.getAlteredObservation(states=torch.tensor(np.reshape(state, [-1, agent.STATE_DIM]), dtype=torch.float32)).detach().cpu().tolist()
...
def evaluatingMuJoCo(eval_times:int=10):
    ...
    print('Summary',
          ...
          ', Avg_Accum_Reward: ', round(avg_return, 2), 
          ', Norm_Avg_Accum_Reward: ', round(avg_norm_return , 2), 
          ', Norm_Std_Accum_Reward: ', round(std_norm_return, 2),
...
dataset = dataset_preprocessing.datasetPreprocessing(dataset=dataset, 

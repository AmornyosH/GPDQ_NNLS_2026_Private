...
# ================================== Others ==================================
from GPDQ_EXACTMATERN2 import my_exact_gp
    ...
    def getAlteredObservation(self, states):
        ...
        with torch.no_grad():
            ...
            for m in range(len(states)):
                ...
                _action_n_tensor = torch.reshape(self.gp_model.y_train_org[m], [-1, self.ACTION_DIM]).repeat(_sampling_size, 1)

                a_i_m_1 = self.predict(state=_state_n_tensor, size=_sampling_size, guide=False)
                
                # Get desired location (a, which max q)
                _q_values_altered = self.q_1(torch.concat([_state_n_tensor, a_i_m_1], dim=1)) 
                _q_values_best = self.q_1(torch.concat([_state_n_tensor, _action_n_tensor], dim=1))

                _q_values = torch.max(_q_values_altered, _q_values_best)
